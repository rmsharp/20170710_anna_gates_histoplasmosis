...
exp_sqlmed_df that has animals in both affected_df and sqlmed_df.
...
```{r first-diagnosis}
exp_sqlmed_df <- merge(affected_df, 
                       sqlmed_first_df[ , c("id", "OBSERVATION_DATE_TM")],
                       by = "id", all = FALSE)
exp_sqlmed_df$sqlmed_first <- format(exp_sqlmed_df$first_noted, 
                                  format = "%Y-%m-%d") > 
  format(exp_sqlmed_df$OBSERVATION_DATE_TM, format = "%Y-%m-%d")
use_sqlmed_date_df <- exp_sqlmed_df[exp_sqlmed_df$sqlmed_first, ]
...
affected_df <- rbind(affected_df, sqlmed_first_df[!sqlmed_first_df$id %in% affected_df$id, ])

```

This will add supplimental data used for some later analyses.

```{r add-supplimenta-data}
...
ctrl_df <- get_ctrl_df(conn, affected_df, arc_species_code)
...
daily_affected_df <- make_daily_location(conn, X_affected, affected_df, housing_types)
...
exp_housing_df <- add_location_type_percents(affected_df, daily_affected_df, 
                                     threshold_min_percent)
ctrl_housing_df <- add_location_type_percents(ctrl_df, daily_ctrl_df, 
                                      threshold_min_percent)

#merged_df <- merge(daily_df, df, by = "id")
affected_df_to_be_melted <- subset(exp_housing_df, select = c("id", "age", "sex", "days_gang", 
...
library(knitr)
caption <- stri_c("Housing history for six of ", length(affected_df_to_be_melted), 
                  " affected animals.")
...
affected_df <- get_housing_type_ratios(conn, affected_df, housing_types, arc_species_code)
...
sex_affected_vs_total_m <- matrix(c(affected_females,
                        total_females,
                        affected_males,
                        total_males), nrow = 2)
sex_affected_vs_total_chisq <- chisq.test(x = sex_affected_vs_total_m)
sex_affected_vs_total_m
sex_affected_vs_total_chisq
...
sex_mce <- get_mce(c('M', 'F'), ntrials = 100000, affected_df$male_prob, 
                   length(affected_df$id[affected_df$sex == 'M']) / nrow(affected_df), stat_f)
sex_mce
alpha <- 0.01
sex_affected_vs_total_rr <- calc_relative_risk(sex_affected_vs_total_m, alpha = alpha)
sex_affected_vs_total_rr

```
The relative risk of a female versus a male of being diagnosed as having 
histoplasmosis is `r round(sex_affected_vs_total_rr$RR, 2)` with a 
`r round((1.0 - sex_affected_vs_total_rr$alpha) * 100.0, 0)` percent confidence of 
`r signif(sex_affected_vs_total_rr$lowervalue, 3)` -- 
`r signif(sex_affected_vs_total_rr$uppervalue, 3)`.
...
```{r get-cbc-and-clinical-chemistry-values}
...
```{r create-id-file-for-genetic-analysis}
ids <- unique(sqlmed_df$animal_id, affected_df$id)
