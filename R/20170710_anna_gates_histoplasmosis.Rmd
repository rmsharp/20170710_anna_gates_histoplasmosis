...
exp_sqlmed <- merge(exp_df, sqlmed_df[ , c("animal_id", "OBSERVATION_DATE_TM")],
                    by.x = "id", by.y = "animal_id",  all = FALSE)
...
chem_df <- get_clin_path(conn, view = "v_clinical_path_chemistry", exp_df)
cbc_df <- get_clin_path(conn, view = "v_clinical_path_cbc", exp_df)
...
`r length(unique(exp_df$id))` affected animals. 
...
ids <- unique(sqlmed_df$animal_id, exp_df$id)
